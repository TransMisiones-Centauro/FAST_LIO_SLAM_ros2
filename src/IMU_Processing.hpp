...
static double toSec(const builtin_interfaces::msg::Time& stamp) {
    const double sec = stamp.sec;
    const double nsec = stamp.nanosec;
...
class ImuProcess
{
 public:
  ...
  ImuProcess(const std::shared_ptr<rclcpp::Node>);
  ~ImuProcess();
...
ImuProcess::ImuProcess(const std::shared_ptr<rclcpp::Node> node)
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1), node(node)
{
  init_iter_num = 1;
  Q = process_noise_cov();
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    if (toSec(tail->header.stamp) < last_lidar_end_time_)    continue;
    ...
    if(toSec(head->header.stamp) < last_lidar_end_time_)
    {
      dt = toSec(tail->header.stamp) - last_lidar_end_time_;
    ...
    double &&offs_t = toSec(tail->header.stamp) - pcl_beg_time;
